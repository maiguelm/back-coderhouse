...
export const removeFromCart = async (req, res) => {
    try {
        ...
        if(!updatedCart) res.json({ message: "No se pudo remover el producto del carrito"})
...
export const deleteCart = async (req, res,next) => {
    try {
        ...
        if(!cartToDelete) res.status(404).json({ error: "el carrito no pudo ser eliminado"});
...
export const updateProdQuantity = async (req, res, next) => {
    try {
        ...
        console.log(`ID del carrito: ${idCart}, ID del producto: ${idProd}, Cantidad: ${quantity}`);

        if (!quantity || isNaN(quantity)) {
            return res.status(400).json({ message: "Cantidad inválida" });
        }

        const updatedCart = await cartServices.updateProdQuantity(idCart, idProd, quantity);
        if (!updatedCart) {
            return res.status(404).json({ message: "No se pudo actualizar la cantidad de productos" });
        }
